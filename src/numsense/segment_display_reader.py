...
class SegmentDisplayReader:
    ...
    def __init__(self, c_thresh=0.6, typeface=DEFAULT_TYPEFACE):
        self.c_thresh = c_thresh

        self.typeface = Typeface(typeface)
        self.typeface.compile()

    def __preprocess_image(self, img, blur_strength=5):
    ...
    def __find_contours(self, processed_img, min_area=128):
        contours, _ = cv.findContours(
            processed_img, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE
        )
        filtered_contours = [cnt for cnt in contours if cv.contourArea(cnt) >= min_area]
        return sorted([cv.boundingRect(cnt) for cnt in filtered_contours])

    def __match_digit_templates(self, digit_roi):
        recognized_digit = None
        best_score = float("-inf")
        for digit, template in self.typeface.templates.items():
            resized_template = cv.resize(template, digit_roi.shape[::-1])
            result = cv.matchTemplate(digit_roi, resized_template, cv.TM_CCOEFF_NORMED)
            score = np.max(result)
            if score > best_score:
                best_score = score
                recognized_digit = digit
        return recognized_digit if best_score >= self.c_thresh else None
    ...
    def __call__(self, img_path):
        ...
        digit_contours = self.__find_contours(processed_img)

        recognized_digits = []
        for x, y, w, h in digit_contours:
            digit_roi = processed_img[y : y + h, x : x + w]
            recognized_digit = self.__match_digit_templates(digit_roi)
